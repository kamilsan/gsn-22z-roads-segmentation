...
class SegmentationModule(pl.LightningModule):
    ...
    def common_step(self, batch: torch.Tensor) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        x, y = batch
        outputs = self(x)
        loss = self.compute_loss(outputs, y.long().squeeze())
        return loss, outputs, y

    def common_test_valid_step(self, batch: torch.Tensor) -> (torch.Tensor, torch.Tensor):
    ...
    def training_step(self, batch: torch.Tensor, batch_idx: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    def validation_step(self, batch: torch.Tensor, batch_idx: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    def test_step(self, batch: torch.Tensor, batch_idx: torch.Tensor) -> dict[str, torch.Tensor]:
        loss, IoU = self.common_test_valid_step(batch)
